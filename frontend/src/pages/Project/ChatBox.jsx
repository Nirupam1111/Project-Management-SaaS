  ...
  return (
    <div className="sticky">
      <div className="border border-gray-400 rounded-lg">
        <h1 className="border-b p-5 font-sans">Chat Box</h1>
        <ScrollArea className="h-[25rem] w-full p-5 flex gap-3 flex-col">
          ...
          {chat.messages?.map((item, i) =>
            item.sender.id == auth.user.id ? (
              ...
              >
                <Avatar>
                  <AvatarFallback className={`bg-green-400 text-white`}>{item.sender.fullName[0]}</AvatarFallback>
                </Avatar>
                <div
                  className={`space-y-2 py-2 px-5 border rounded-ss-2xl rounded-e-xl bg-green-400`}
                >
                  <p className="font-sans text-white text-sm">{item.sender?.fullName}</p>
        ...
        <div className="relative p-0">
          <Input
            ...
            className="py-7 border-t border-gray-300 outline-none  focus:outline-none focus:ring-0 rounded-none border-b-0 border-x-0 font-sans"
          />
          <Button
